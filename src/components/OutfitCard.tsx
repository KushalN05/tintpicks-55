...
const OutfitCard: React.FC<OutfitCardProps> = ({ outfit, children, animateProps }) => (
  <div
    className="shadow-2xl rounded-3xl overflow-hidden relative w-full max-w-md aspect-[3/4] bg-white border-2 border-border"
    {...animateProps}
  >
    <img
      src={outfit.imageUrl}
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <span className="ml-1 text-sm text-foreground">{outfit.dominantColor}</span>
      </div>
    </div>
    {children}
  </div>
